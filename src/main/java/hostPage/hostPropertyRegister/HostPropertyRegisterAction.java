...
public class HostPropertyRegisterAction {
	...
	public ResponseData execute(HttpServletRequest request, HttpServletResponse response) throws UnsupportedEncodingException, ServletException, IOException  {
		...
		try {
			fileName = upload(request.getPart("image"));
		} catch (Exception e) {
			e.printStackTrace();
		}
		if(fileName == null) {
	...
	private String upload(Part filePart) {
		String fileName = "";
		try {
			...
			fileName = "pro001" + extension;
			String SAVE_DIR12 = "C:\\Users\\hayar\\property_photo"; // 저장 경로
			// 실제 저장 경로
